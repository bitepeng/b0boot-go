...
func run() {
	//注册应用ui路由
	engine.Print(aurora.Green("config live"), aurora.BrightCyan(config.Live))
	...
	engine.Print(aurora.Green("server gateway start at "), aurora.BrightBlue(config.ListenAddr))
	//启动httpd服务
